...
- ได้แก่ CMakeLists.txt (ไฟล์หลัก), main/CMakeLists.txt และไฟล์โค้ดหลักใน main/ เช่น hello_esp32.c ซึ่งมีฟังก์ชัน app_main() เป็นจุดเริ่มต้นของโปรแกรม
2. ความแตกต่างระหว่าง `hello_esp32.bin` และ `hello_esp32.elf` คืออะไร?
- ไฟล์ .bin คือโปรแกรมที่พร้อมแฟลชลงบอร์ดจริง ส่วน .elf มีข้อมูลสำหรับ debug ใช้ตรวจสอบและแก้ไขโปรแกรม
3. คำสั่ง `idf.py set-target` ทำอะไร?
- ใช้กำหนดชิปเป้าหมาย เช่น esp32 หรือ esp32s3 เพื่อให้ระบบตั้งค่าการ build ให้ตรงกับฮาร์ดแวร์
4. โฟลเดอร์ `build/` มีไฟล์อะไรบ้าง?
- มีผลลัพธ์จากการ build เช่น hello_esp32.bin, hello_esp32.elf, โฟลเดอร์ bootloader/, partition_table/ และไฟล์ตั้งค่าการแฟลช
5. การใช้ `vTaskDelay()` แทน `delay()` มีความสำคัญอย่างไร?
- vTaskDelay() หน่วงเวลาแบบไม่บล็อก ทำให้ task อื่นยังทำงานได้ ส่วน delay() บล็อก CPU ทำให้ระบบหยุดชั่วคราวทั้งหมด
